...
| 中英翻译 | 列表「翻译列表」总开关：随滚动自动翻译视口内卡片（标题+摘要，失败可重试）；阅读页「翻译全文」（标题+正文）：Google 免费接口（gtx），跟随 GUI 语言自动互译，Host 缓存 24h |
...
3. 列表顶部点「翻译列表」开启总开关：随滚动自动翻译视口内的卡片标题+摘要（翻译失败卡片显示「重试」，再点「关闭翻译」恢复原文）；点击卡片进入站内阅读，正文加载后右上角「翻译全文」把标题+全文译成 GUI 语言（可再点「显示原文」切换），底部「原文链接」在新标签页打开原站。
